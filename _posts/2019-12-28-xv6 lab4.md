---
layout: post
title:  "6.828 Lab 4: Preemptive Multitasking"
...
sys_page_map(envid_t srcenvid, void *srcva, envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((uintptr_t) srcva >= UTOP || (uintptr_t) srcva % PGSIZE || ((~perm) & (PTE_U | PTE_P)))
        return -E_INVAL;

	if ((uintptr_t) dstva >= UTOP || (uintptr_t) dstva % PGSIZE || ((~perm) & (PTE_U | PTE_P)))
        return -E_INVAL;

	pte_t* pte;
	struct PageInfo* p = page_lookup(src_e->env_pgdir, srcva, &pte);
	if (p == NULL || (((~(*pte)) & PTE_W) && (perm & PTE_W)))
		return -E_INVAL;
...
1. 内核将页面错误传播到`_pgfault_upcall`，后者调用`fork()`的`pgfault()`处理函数。
