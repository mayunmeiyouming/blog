...
要测试你的映射，请尝试打印出设备状态寄存器（第13.4.2节）。这是一个4字节的寄存器，从寄存器空间的字节8开始。你应该得到`0x80080783`，这表示全双工链接的速度高达1000 MB/s。

提示：你将需要很多常量，例如寄存器的位置和位掩码的值。尝试将这些内容从开发人员手册中复制出来很容易出错，而错误可能导致痛苦的调试。我们建议把QEMU的`e1000_hw.h`作为指导。我们不建议逐字复制它，因为它定义的内容远远超出你的实际需要，并且可能无法按照你需要的方式进行定义，但这是一个很好的起点。
...
为了更彻底地测试你的代码，我们提供了一个名为`echosrv`的守护程序，该守护程序在端口7上运行了回显服务器，该服务器将回显通过TCP连接发送的所有内容。使用`make E1000_DEBUG=TX,TXERR,RX,RXERR,RXFILTER run-echosrv-nox`在一个终端中启动回显服务器，在另一个终端中使`nc-7`与之连接。服务器应回显你输入的每一行。每次模拟的E1000收到数据包时，QEMU都应在控制台上打印以下内容：
...
完成Web服务器后，启动Web服务器（运行run-httpd-nox），然后将您喜欢的浏览器指向http：// host：port / index.html，其中host是运行QEMU的计算机的名称（如果您在雅典娜上运行QEMU，请使用hostname.mit.edu（如果您在同一台计算机上运行Web浏览器和QEMU，则主机名是雅典娜在hostname命令的输出；如果您在同一台计算机上运行Web浏览器和QEMU，则为localhost），并且port是报告的端口号通过make which-ports进入Web服务器，您应该看到在JOS中运行的HTTP服务器提供的网页。

在这一点上，你的成绩应该达到105/105。