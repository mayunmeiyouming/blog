...
int attachfn_enable(struct pci_func *pcif) {
    ...
    *(volatile uint32_t *)(bar_va + TDLEN) = TXRING_LEN;
...
int try_transmit(physaddr_t pa, size_t len)
{
    uint32_t tdt = *(volatile uint32_t *) (bar_va + 0x3818);
    ...
    memcpy(&(pbuf[tdt]), &pa, len);
    uint32_t next = (tdt + 1) % TXRING_LEN;
    *(volatile uint32_t *) (bar_va + 0x3818) = next;
    return 0;
}
```
